...
export class TombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    // Uniswap V2 Pair
    this.TOMBWAVAX_LP = new Contract(externalTokens['MSC-AVAX-LP'][0], IUniswapV2PairABI, provider);
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('MSC') ? this.TOMB : this.TSHARE;
    const isTomb = name.startsWith('MSC');
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'MSC' ? await this.getTombStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (earnTokenName === 'MSC') {
      if (!contractName.endsWith('TombRewardPool')) {
        const rewardPerSecond = await poolContract.mscPerSecond();
  ...
  async earnedFromBank(
    ...
    try {
      if (earnTokenName === 'MSC') {
        // problem is pendingMSC isnt a function since the abi still says pendingTOMB
        return await pool.pendingMSC(poolId, account);
      } else {
  ...
  async watchAssetInMetamask(assetName: string): Promise<boolean> {
    ...
    if (ethereum && ethereum.networkVersion === config.chainId.toString()) {
      ...
      if (assetName === 'MSC') {
        asset = this.TOMB;
        assetUrl = 'https://raw.githubusercontent.com/krypto-dev/msc-finance/main/src/assets/img/MSC.png';
      } else if (assetName === 'MSHARE') {
        asset = this.TSHARE;
        assetUrl = 'https://raw.githubusercontent.com/krypto-dev/msc-finance/main/src/assets/img/MSHARE.png';
      } else if (assetName === 'MBOND') {
        asset = this.TBOND;
        assetUrl = 'https://raw.githubusercontent.com/krypto-dev/msc-finance/main/src/assets/img/MBOND.png';
      } else if (assetName === 'MSC-WAVAX') {
        asset = this.TOMBWAVX;
        assetUrl = 'https://raw.githubusercontent.com/krypto-dev/msc-finance/main/src/assets/img/MSCWAVX.png';
      } else {
        asset = this.TSHAREWAVX;
        assetUrl = 'https://raw.githubusercontent.com/krypto-dev/msc-finance/main/src/assets/img/MSHAREWAVX.png';
      }
