...
contract Msc is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 36h genesis pools
    ...
    /**
     * @notice Constructs the MSC ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MoonShine", "MSC") {
        // Mints 1 MSC to contract creator for initial pool setup
    ...
    function _getMscPrice() internal view returns (uint256 _mscPrice) {
        try IOracle(mscOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Msc: failed to fetch MSC price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _mscPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_mscPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMscOracle(address _mscOracle) public onlyOperatorOrTaxOffice {
        require(_mscOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MSC to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MSC to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMscPrice = _getMscPrice();
            currentTaxRate = _updateTaxRate(currentMscPrice);
            if (currentMscPrice < burnThreshold) {
                burnTax = true;
            }
        }
